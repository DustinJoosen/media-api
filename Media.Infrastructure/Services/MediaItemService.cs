...
namespace Media.Infrastructure.Services
{
	public class MediaItemService : IMediaItemService
	{
		...
		public async virtual Task<UploadMediaItemResponse> UploadMediaItem(string token,
			UploadMediaItemRequest mediaItemReq, CancellationToken cancellationToken = default)
		{
			...
			if (!tokenInfo.Permissions.HasFlag(AuthTokenPermissions.CanCreate))
				throw new UnauthorizedException(
					ErrorMessages.CouldNotActionMediaMissingPermission("upload", "CanCreate"));
			...
			using var transaction = await this._context.Database
		...
		{
			var item = await this._context.MediaItems.FindAsync(id, cancellationToken);
			if (item == null)
			...
			CancellationToken cancellationToken = default)
		{
			...
			if (!tokenInfo.Permissions.HasFlag(AuthTokenPermissions.CanDelete))
				throw new UnauthorizedException(
					ErrorMessages.CouldNotActionMediaMissingPermission("delete", "CanDelete"));

			if (item.CreatedByToken != token)
				throw new UnauthorizedException(
					ErrorMessages.CouldNotActionMediaUserIsNotOwner("delete"));
		...
		/// <param name="token">Token to proof you have the correct deletion rights.</param>
		/// <param name="cancellationToken">Token to observe for cancellation requests.</param>
		public async Task ModifyById(Guid id, ModifyMediaItemRequest modifyMediaItemReq,
			string token, CancellationToken cancellationToken = default)
		{
			...
			if (!tokenInfo.Permissions.HasFlag(AuthTokenPermissions.CanModify))
				throw new UnauthorizedException(
					ErrorMessages.CouldNotActionMediaMissingPermission("modify", "CanModify"));

			if (item.CreatedByToken != token)
				throw new UnauthorizedException(
					ErrorMessages.CouldNotActionMediaUserIsNotOwner("modify"));
		...
		internal void CheckFormFileValid(IFormFile formFile)
		{
			if (formFile.Length > this._options.MaxFileSize)
				throw new BadRequestException(
					ErrorMessages.FileTooLarge(this._options.MaxFileSize));

			var extension = Path.GetExtension(formFile.FileName);
			if (this._options.BlockedFileExtensions
				.Any(be => be.Equals(extension, StringComparison.OrdinalIgnoreCase)))
			{
