	...
	public class MediaItemController : ControllerBase
	{
			...
			CancellationToken cancellationToken = default)
		{
			var mediaFile = await this._mediaItemService
				.GetMediaItemFileStreamPreview(id, cancellationToken);
		...
		{
			var mediaFile = await this._mediaItemService
				.GetMediaItemFileStreamDownload(id, cancellationToken);
		...
		[Route("upload")]
		[TokenValid]
		public async Task<UploadMediaItemResponse> UploadMediaItem(
			[FromForm] UploadMediaItemRequest mediaItemReq, 
			CancellationToken cancellationToken = default)
		{
			string token = this.Request.Headers.Authorization.ToString();
			return await this._mediaItemService.UploadMediaItem(token, mediaItemReq, 
				cancellationToken);
		}
		...
		{
			string token = this.Request.Headers.Authorization.ToString();
			await this._mediaItemService.ModifyById(id, modifyMediaItemReq, token, 
				cancellationToken);
